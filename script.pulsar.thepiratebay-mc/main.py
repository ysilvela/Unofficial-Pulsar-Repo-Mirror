...
def extract_magnets(data):
    try:
        ...
        size = re.findall('class="size-row">(.*?)B', data) # list the size
        cont = 0
...
def search(query):
    ...
    url_search = "%s/search.php?q=%s&Torrent_sort=seeders.desc" % (settings.url, query.replace(' ', '%20'))  # change in each provider
